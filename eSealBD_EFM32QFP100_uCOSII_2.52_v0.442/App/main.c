...
CPU_BOOL BDshenzhouTest(void)
{
    CPU_INT16U i;
    CPU_INT16U send_length;
    unsigned char send_buffer[500];
    CPU_INT8U reply[256];
    unsigned char send_data[]="123456";
    COM3RxFIFOClear();
    BDComDATAPack(send_buffer,send_data,sizeof(send_data)-1);
    BDComSendData(send_buffer, 31);
    OSTimeDly(3000);
    while (COM3RxFIFOIsEmpty() == DEF_FALSE)
    {
       reply[i]  = COM3RxFIFOOut();
       i++;
