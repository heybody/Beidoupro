...
extern sGnssInfoTypeDef  GNSSGNInfo;
...
{
    .power  = DEF_ON,\
    .safe   = ESEALBD_STATE_SAFE_LOCKED,\
};
...
void EsealBDQiuUpdate(void)
{
    ...
    if(count==30) //1
    {
        ...
        package.latitude= GNSSGNInfo.lonlat.latitude_h*10000+GNSSGNInfo.lonlat.latitude_l;/*填入经度*/
        package.longitude=GNSSGNInfo.lonlat.latitude_h*10000+GNSSGNInfo.lonlat.longitude_l;/*填入纬度*/
        package.latitudedirection=GNSSGNInfo.lonlat.latitude_direction;
        package.longitudedirection=GNSSGNInfo.lonlat.longitude_direction;
        package.humidityset=EsealBDSensor.humidity;/*填入湿度阈值*/
        package.temperatureset=EsealBDSensor.temperature;/*填入温度阈值*/
        package.shakeset=EsealBDSensor.shake;
        package.temperature=EsealBDPresentSensor.temperature;/*填入温度*/
        package.humidity=EsealBDPresentSensor.humidity;/*填入湿度*/
        package.shake=EsealBDPresentSensor.shake;
        package.deviceid=NToHL(EsealBDID);
        package.reveiceflag=1;
        package.openflag=state.sw;
        BDComDATAPack(send_data, (CPU_INT8U *)&package, sizeof(EsealQiuupdatePackage));
        BDComSendData(send_data, sizeof(EsealQiuupdatePackage)+ 1 + BD_COM_PACK_DATA_SIZE);
    }
}
           ...
           if ((GprsSocketStatusGet() == DEF_ON) && (count < ESEALBD_GPRS_RESEND_COUNT))
            ...
            switch (channel)
            {
                ...
                case ESEALBD_WORK_CH_GPRS: GprsBuffDataOut(item.data, item.len, &err);
                                           break;
                case ESEALBD_WORK_CH_BD:   BDBuffDataOut(item.data, item.len);
                                           break;
                case ESEALBD_WORK_CH_GPRS_BD:GprsBuffDataOut(item.data, item.len, &err);
                                            BDBuffDataOut(item.data, item.len);
                                           break;
...
/**
 * [EsealBDUpdataExtra  description]
 * @param type  [ESEALBD_UPDATA_IRQ_TEMPERATURE]
 * @param delay [0]
 * @param count [1]
 * @param com   [EsealBuffDataIn]
 */
...
//////////////////////////////////////////
        ...
        EsealBDGetFix(&fix);//禄帽脠隆脳酶卤锚脨脜脧垄
