...
//    ASSERT(ptRingBuf != NULL);
    ...
    //
    return((((ulWrite + 1) % ptRingBuf->ulSize) == ulRead) ? 1 : 0);
}
...
unsigned char
RingBufProbe(tRingBufObject *ptRingBuf, unsigned long ulSpan)
{
    unsigned char ucTemp;
		unsigned long ulIndex;

    ulIndex = ptRingBuf->ulReadIndex + ulSpan;
		//
    // Correct for wrap. We use a loop here since we don't want to use a
    // modulus operation with interrupts off but we don't want to fail in
    // case ulDelta is greater than ulSize (which is extremely unlikely but...)
    //
    while(ulIndex >= ptRingBuf->ulSize)
    {
        ulIndex -= ptRingBuf->ulSize;
    }

    //
    // read the data byte.
    //
    ucTemp = ptRingBuf->pucBuf[ulIndex];
